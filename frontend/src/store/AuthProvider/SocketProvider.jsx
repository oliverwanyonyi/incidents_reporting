...
const SocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    socket.off("agents_online").on("agents_online", (data) => {
      setAuthoritiesOnline(data);
    });

    socket.off("users_online").on("users_online", (data) => {
      console.log(data);
      setUsersOnline(data);
    });
    ...
    socket.off("agent_busy").on("agent_busy", (data) => {
      ...
      infoToast(
        `All Authorities are busy, your postion in queue is ${data.position} please try again later after ${data.maxWaitingTime} minutes`
      );
    });
